#!/usr/bin/env python3
"""
Scrape CBS Sports to calculate QB passing yards defensive rankings
"""
...
def scrape_team_qb_passing_yards_stats(team_abbr, max_week=None):
    """Scrape QB passing yards allowed for a specific team through a certain week"""
    ...
    try:
        ...
        for row in rows:
            ...
            try:
                ...
                # Passing yards is in column 5 (based on debug output from earlier: Att, Cmp, Yd, TD, Int, Rate)
                # Actually from debug: 2=Att, 3=Cmp, 4=Yd, 5=TD, 6=Int, 7=Rate
                passing_yards_text = cells[4].get_text().strip()
                
                if passing_yards_text and passing_yards_text.lstrip('-').isdigit():
                    passing_yards = int(passing_yards_text)
        ...
        return {
            ...
            'games': games_count,
            'per_game': total_passing_yards / games_count if games_count > 0 else 0,
            'weeks': weeks_found
        }
...
def main():
    ...
    print("=" * 80)
    print("SCRAPING CBS SPORTS: QB Passing Yards")
    if max_week:
    ...
    for team_abbr, team_name in NFL_TEAMS.items():
        ...
        stats = scrape_team_qb_passing_yards_stats(team_abbr, max_week)
        
        if stats:
            team_stats.append(stats)
            print(f"✅ {stats['total_passing_yards']} yards in {stats['games']} games")
        else:
            print("❌ Failed")
    ...
    for team_data in team_stats:
        print(f"{team_data['rank']:<6} {team_data['team']:<35} {team_data['total_passing_yards']:<12} {team_data['games']:<8} {team_data['per_game']:<10.2f}")
    ...
    if highlight_team:
        ...
        for team_data in team_stats:
            if team_data['team_abbr'] == highlight_team:
                ...
                print(f"Total QB Passing Yards Allowed: {team_data['total_passing_yards']}")
