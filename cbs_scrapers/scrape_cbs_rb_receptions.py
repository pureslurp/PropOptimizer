#!/usr/bin/env python3
"""
Scrape CBS Sports to calculate RB receptions defensive rankings
"""
...
def scrape_team_rb_receptions_stats(team_abbr, max_week=None):
    """Scrape RB receptions allowed for a specific team through a certain week"""
    ...
    try:
        ...
        for row in rows:
            ...
            try:
                ...
                # For RB table: 0=Week, 1=Team, 2-5=Rushing stats, 6=Targets, 7=Recpt, 8=Yd, 9=Avg, 10=TD
                # Receptions is in column 7
                receptions_text = cells[7].get_text().strip()
                
                if receptions_text and receptions_text.lstrip('-').isdigit():
                    receptions = int(receptions_text)
        ...
        return {
            ...
            'games': games_count,
            'per_game': total_receptions / games_count if games_count > 0 else 0,
            'weeks': weeks_found
        }
...
def main():
    ...
    print("=" * 80)
    print("SCRAPING CBS SPORTS: RB Receptions")
    if max_week:
    ...
    for team_abbr, team_name in NFL_TEAMS.items():
        ...
        stats = scrape_team_rb_receptions_stats(team_abbr, max_week)
        
        if stats:
            team_stats.append(stats)
            print(f"✅ {stats['total_receptions']} receptions in {stats['games']} games")
        else:
            print("❌ Failed")
    ...
    # Sort by per-game receptions (ascending = best defense = rank 1)
    ...
    # Display rankings
    print(f"{'Rank':<6} {'Team':<35} {'Total Rec':<12} {'Games':<8} {'Per Game':<10}")
    print("-" * 80)
    
    for team_data in team_stats:
        print(f"{team_data['rank']:<6} {team_data['team']:<35} {team_data['total_receptions']:<12} {team_data['games']:<8} {team_data['per_game']:<10.2f}")
    ...
    if highlight_team:
        ...
        for team_data in team_stats:
            if team_data['team_abbr'] == highlight_team:
                ...
                print(f"Total RB Receptions Allowed: {team_data['total_receptions']}")
